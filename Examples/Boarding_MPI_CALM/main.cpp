...
void master(char *);
void slave();
void CALM(Numeric sm, Numeric idt, Numeric isc, Numeric tssc, Numeric rt, int ID);

using namespace std;

int id;    // process rank
int np;    // number of MPI processes
int NUM = 1000;   // number of tasks to be processed  (given 
...
*/
		...
		//sprintf(buffer,"%d %d\n", work, np);
		//system(buffer);
		CALM(work[1], work[2], work[3], work[4], work[5], work[0]);
...
void CALM(Numeric sm, Numeric idt, Numeric isc, Numeric tssc, Numeric rt, int ID){
  ...
  Numeric min_TSSC = 0.2f;
  Numeric min_RT = 0.5f;

  Numeric max_SM = 1.3f;
  Numeric max_IDT = 1.5f;
  Numeric max_ISC = 0.8f;
  Numeric max_TSSC = 0.6f;
  Numeric max_RT = 1.6f;
  ...
  Numeric this_TSSC = (max_TSSC - min_TSSC) * tssc + min_TSSC;
  ...
  output<<"Running simulation with SimID= "<<ID<<", IDT= "<<this_IDT<<", ISC= "<<this_ISC<<", TSSC= "<<this_TSSC<<", RT= "<<this_RT<<", SM= "<<this_SM<<endl;
  Simulation sim = Simulation(this_SM, 0.2f, this_IDT, this_ISC, this_TSSC, this_RT, ID);
