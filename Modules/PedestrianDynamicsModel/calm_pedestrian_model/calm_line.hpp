...
struct Line {
  ...
  [[nodiscard]] inline constexpr bool doesIntersect(const Line& other) const {
    ...
    if (o1 == 0 && checkIfOnLineSegment(p1, p2, other.p1))
      return true;
    if (o2 == 0 && checkIfOnLineSegment(p1, p2, other.p2))
      return true;
    if (o3 == 0 && checkIfOnLineSegment(other.p1, other.p2, p1))
      return true;
    if (o4 == 0 && checkIfOnLineSegment(other.p1, other.p2, p2))
      return true;
  ...
  [[nodiscard]] inline constexpr bool isPointOn(VIPRA::f3d point) const {
    return point.x <= std::max(p1.x, p2.x) && point.y <= std::max(p1.y, p2.y) && point.x >= std::min(p1.x, p2.x) &&
