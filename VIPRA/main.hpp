...
std::unique_ptr<HumanBehaviorModel> generateHumanBehaviorModel(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<PedestrianLoader> generatePedestrianLoader(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<Simulation>          generateSimulation(const std::string&        id,
                                                        const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<ConfigurationReader> generateConfigurationReader(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<ObstacleSet>      generateObstacleSet(const std::string&        id,
                                                      const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<PedestrianSet>    generatePedestrianSet(const std::string&        id,
                                                        const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<OutputDataWriter> generateOutputDataWriter(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<MapLoader>               generateMapLoader(const std::string&        id,
                                                           const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<PolicyModel>             generatePolicyModel(const std::string&        id,
                                                             const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<Goals>                   generateGoals(const std::string&        id,
                                                       const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<SimulationOutputHandler> generateSimulationOutputHandler(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
std::unique_ptr<PedestrianDynamicsModel> generatePedestrianDynamicsModel(
    const std::string& id, const VIPRA::CONFIG::Map& configMap);
...
void parallel_main(VIPRA::CONFIG::Map simconfig, VIPRA::CONFIG::Map moduleparams) {
...
#endif
  VIPRA::CONFIG::Map human_behavior_modelConfig =
      extractConfigMap("human_behavior_model");
  ...
  VIPRA::CONFIG::Map configuration_readerConfig =
      extractConfigMap("configuration_reader");
  ...
  VIPRA::CONFIG::Map simulation_output_handlerConfig =
      extractConfigMap("simulation_output_handler");
  VIPRA::CONFIG::Map pedestrian_dynamics_modelConfig =
      extractConfigMap("pedestrian_dynamics_model");
  std::unique_ptr<HumanBehaviorModel> human_behavior_model = generateHumanBehaviorModel(
      simulationJsonConfig["modules"]["human_behavior_model"].asString(),
      human_behavior_modelConfig);
  std::unique_ptr<PedestrianLoader> pedestrian_loader = generatePedestrianLoader(
      simulationJsonConfig["modules"]["pedestrian_loader"].asString(),
      pedestrian_loaderConfig);
  std::unique_ptr<Simulation> simulation = generateSimulation(
      simulationJsonConfig["modules"]["simulation"].asString(), simulationConfig);
  std::unique_ptr<ConfigurationReader> configuration_reader = generateConfigurationReader(
      simulationJsonConfig["modules"]["configuration_reader"].asString(),
      configuration_readerConfig);
  std::unique_ptr<ObstacleSet> obstacle_set = generateObstacleSet(
      simulationJsonConfig["modules"]["obstacle_set"].asString(), obstacle_setConfig);
  std::unique_ptr<PedestrianSet> pedestrian_set = generatePedestrianSet(
      simulationJsonConfig["modules"]["pedestrian_set"].asString(), pedestrian_setConfig);
  std::unique_ptr<OutputDataWriter> output_data_writer = generateOutputDataWriter(
      simulationJsonConfig["modules"]["output_data_writer"].asString(),
      output_data_writerConfig);
  std::unique_ptr<MapLoader> map_loader = generateMapLoader(
      simulationJsonConfig["modules"]["map_loader"].asString(), map_loaderConfig);
  std::unique_ptr<PolicyModel> policy_model = generatePolicyModel(
      simulationJsonConfig["modules"]["policy_model"].asString(), policy_modelConfig);
  std::unique_ptr<Goals> goals =
      generateGoals(simulationJsonConfig["modules"]["goals"].asString(), goalsConfig);
  std::unique_ptr<SimulationOutputHandler> simulation_output_handler =
      generateSimulationOutputHandler(
          simulationJsonConfig["modules"]["simulation_output_handler"].asString(),
          simulation_output_handlerConfig);
  std::unique_ptr<PedestrianDynamicsModel> pedestrian_dynamics_model =
      generatePedestrianDynamicsModel(
          simulationJsonConfig["modules"]["pedestrian_dynamics_model"].asString(),
          pedestrian_dynamics_modelConfig);
  spdlog::info("Generating Modules");
  ...
  auto stopTime = timer.stop();
  spdlog::debug("Obstacle Set Initialization: {}", stopTime);
  ...
  stopTime = timer.stop();
  spdlog::debug("Loading Pedestrians: {}", stopTime);
  ...
  stopTime = timer.stop();
  spdlog::debug("Pedestrian Set Initialization: {}", stopTime);
  ...
  stopTime = timer.stop();
  spdlog::debug("Goals Initialization: {}", stopTime);
  ...
  stopTime = timer.stop();
  spdlog::debug("Pedestrian Model Initialization: {}", stopTime);
  ...
  stopTime = timer.stop();
  spdlog::debug("Behavior Model Initialization: {}", stopTime);
  ...
  simulation->run(*goals, *pedestrian_set, *obstacle_set, *pedestrian_dynamics_model,
                  *human_behavior_model, *policy_model, *output_data_writer,
...
std::unique_ptr<HumanBehaviorModel> generateHumanBehaviorModel(
  ...
  if (id == "e3y1yG6PSjrWzsptf6jHdfBElwFugQ") {
    std::unique_ptr<HumanBehaviorModel> human_behavior_model =
        std::make_unique<HumanBehaviorModel>();
    spdlog::info(
        "Configuring human_behavior_model Module: ID: e3y1yG6PSjrWzsptf6jHdfBElwFugQ");

    human_behavior_model->configure(configMap);
    return human_behavior_model;
  }
  spdlog::error(
      "No Valid Module of Type: human_behavior_modelWas Chosen. Provided ID: {} ", id);
  exit(1);
  return nullptr;
}
std::unique_ptr<PedestrianLoader> generatePedestrianLoader(
  ...
  if (id == "8oGawQjwJZutb4Us6lDU7R") {
    std::unique_ptr<JsonPedestrianLoader> json_pedestrian_loader =
        std::make_unique<JsonPedestrianLoader>();
  ...
  spdlog::error("No Valid Module of Type: pedestrian_loaderWas Chosen. Provided ID: {} ",
                id);
  exit(1);
  return nullptr;
}
...
std::unique_ptr<ConfigurationReader> generateConfigurationReader(
  ...
  if (id == "5E4V21CyOwU5iMBfH97qlFDn6DlAlf") {
    std::unique_ptr<ConfigurationReader> configuration_reader =
        std::make_unique<ConfigurationReader>();
    spdlog::info(
        "Configuring configuration_reader Module: ID: 5E4V21CyOwU5iMBfH97qlFDn6DlAlf");

    configuration_reader->configure(configMap);
    return configuration_reader;
  }
  spdlog::error(
      "No Valid Module of Type: configuration_readerWas Chosen. Provided ID: {} ", id);
  exit(1);
  return nullptr;
}
std::unique_ptr<ObstacleSet> generateObstacleSet(const std::string&        id,
  ...
  if (id == "MTpemEr4jv5XTvgwO7q54Qco97Pnt4") {
    std::unique_ptr<PassengerVehicleObstacleSet> passenger_vehicle_obstacle_set =
        std::make_unique<PassengerVehicleObstacleSet>();
...
std::unique_ptr<PedestrianSet> generatePedestrianSet(
    const std::string& id, const VIPRA::CONFIG::Map& configMap) {
  spdlog::info("Creating pedestrian_set Implementation");

  if (id == "jLcPe7ZP8G15AFH5vPb6Wz2DrIVT94") {
    std::unique_ptr<CalmPedestrianSet> calm_pedestrian_set =
        std::make_unique<CalmPedestrianSet>();
  ...
  spdlog::error("No Valid Module of Type: pedestrian_setWas Chosen. Provided ID: {} ",
                id);
  exit(1);
  return nullptr;
}
std::unique_ptr<OutputDataWriter> generateOutputDataWriter(
  ...
  if (id == "esBm2A3I0yIbGvzW5bLY0yKHJTzVCj") {
    std::unique_ptr<JSONTimestepWriter> json_timestep_writer =
        std::make_unique<JSONTimestepWriter>();
    spdlog::info(
        "Configuring output_data_writer Module: ID: esBm2A3I0yIbGvzW5bLY0yKHJTzVCj");
  ...
  if (id == "f8qt7eLCczzFMjToJBsA5qYUz1A8E6") {
    ...
    spdlog::info(
        "Configuring output_data_writer Module: ID: f8qt7eLCczzFMjToJBsA5qYUz1A8E6");

    json_writer->configure(configMap);
    return json_writer;
  }
  spdlog::error("No Valid Module of Type: output_data_writerWas Chosen. Provided ID: {} ",
                id);
  exit(1);
  return nullptr;
}
...
std::unique_ptr<SimulationOutputHandler> generateSimulationOutputHandler(
  ...
  if (id == "RfH40LtLJjHOlPIEeTj0i6ghBygQsZ") {
    std::unique_ptr<TimestepConsoleLogger> timestep_console_logger =
        std::make_unique<TimestepConsoleLogger>();
    spdlog::info(
        "Configuring simulation_output_handler Module: ID: "
        "RfH40LtLJjHOlPIEeTj0i6ghBygQsZ");
  ...
  if (id == "0bQVYYyskvxYq6YW4J67z73UB6GbnU") {
    std::unique_ptr<TimestepOutputHandler> timestep_output_handler =
        std::make_unique<TimestepOutputHandler>();
    spdlog::info(
        "Configuring simulation_output_handler Module: ID: "
        "0bQVYYyskvxYq6YW4J67z73UB6GbnU");

    timestep_output_handler->configure(configMap);
    return timestep_output_handler;
  }
  spdlog::error(
      "No Valid Module of Type: simulation_output_handlerWas Chosen. Provided ID: {} ",
      id);
  exit(1);
  return nullptr;
}
std::unique_ptr<PedestrianDynamicsModel> generatePedestrianDynamicsModel(
  ...
  if (id == "VbGjNW5NCkOmKAxvFmz5KwUV2zz469") {
    std::unique_ptr<CalmPedestrianModel> calm_pedestrian_model =
        std::make_unique<CalmPedestrianModel>();
    spdlog::info(
        "Configuring pedestrian_dynamics_model Module: ID: "
        "VbGjNW5NCkOmKAxvFmz5KwUV2zz469");

    calm_pedestrian_model->configure(configMap);
    return calm_pedestrian_model;
  }
  spdlog::error(
      "No Valid Module of Type: pedestrian_dynamics_modelWas Chosen. Provided ID: {} ",
      id);
  exit(1);
  return nullptr;
}
