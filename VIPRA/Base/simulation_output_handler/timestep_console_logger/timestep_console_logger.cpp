...
bool TimestepConsoleLogger::isOutputCriterionMet(const PedestrianSet&, const ObstacleSet&,
                                                 const Goals&,
                                                 VIPRA::t_step currTimestep) {
  return (currTimestep % frequency == 0);
}

void TimestepConsoleLogger::writeOutput(OutputDataWriter&,
  ...
  for (VIPRA::idx i = 0; i < coordCnt; ++i) {
    spdlog::debug("Pedestrian {}: x:{}, y:{}, z:{}", std::to_string(i), coords.at(i).x,
