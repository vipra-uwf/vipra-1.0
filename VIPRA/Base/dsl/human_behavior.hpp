...
class HumanBehavior {
 public:
  ...
  virtual void update(const ObstacleSet&, const PedestrianSet&, const Goals&, VIPRA::delta_t);

  // Select a specific pedestrian for this behavior
  virtual bool select(const ObstacleSet&, const PedestrianSet&, const Goals&, VIPRA::idx);

  // Tell the simulation whether or not this behavior will decide their speed (or anything else for that matter).
  virtual bool decide(const ObstacleSet&, const PedestrianSet&, const Goals&, VIPRA::idx);

  // Perform the action associated with this behavior
  virtual void act(const ObstacleSet&   obstacleSet,
