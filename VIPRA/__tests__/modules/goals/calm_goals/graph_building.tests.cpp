...
#include "Goals/calm_goals/adjacencyGraph.hpp"

namespace graph_build_TESTS {

class testGraph : public CalmPath::Graph {
 public:
  std::optional<VIPRA::f3d> testLineIntersect(const VIPRA::f3d& start1,
...
testLineIntersect(void) {
  ...
  TEST_ASSERT((test1.value() == VIPRA::f3d{-3, -7}));
  ...
  TEST_ASSERT((test7.value() == VIPRA::f3d{6, 6}));
}
...
testQuadIntersect(void) {
  ...
  TEST_ASSERT(TEST_DIMENSIONS(test1.value().point, VIPRA::f3d{0, -1}, 0.001));
  TEST_ASSERT((test1.value().direction == VIPRA::f3d{0, -1}));

  const auto test2 = graph.testQuadEdgeIntersect(&quad1, {-2, -7}, {2, 5});
  TEST_ASSERT(test2.has_value());
  TEST_ASSERT(TEST_DIMENSIONS(test2.value().point, VIPRA::f3d{(2.0 / 3.0), 1}, 0.001));
  TEST_ASSERT(TEST_DIMENSIONS(test2.value().direction, VIPRA::f3d{0, 1}, 0.001));
}
