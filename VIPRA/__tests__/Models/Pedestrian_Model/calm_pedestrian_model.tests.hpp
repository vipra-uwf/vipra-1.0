...
void
ModelReturnsCorrectDistanceMatrix(void) {
...
void
ModelCalculatesCorrectNearestNeighbor(void) {
  CalmPedestrianSet   pedestrianSet;
  CalmPedestrianModel pedestrianModel;
  AirplaneObstacleSet obs;
  CalmGoals           goals;
...
void
ModelCalculatesCorrectPropulsion(void) {
  ...
  pedSet.initialize(CALM_PEDESTRIAN_VIPRA::EntitySet);
  ...
  for (int i = 0; i < pedSet.getNumPedestrians(); i++) {
    TEST_CHECK(pedModel.getPropulsionForces().at(i).x == pedModel.getPropulsionForces().at(i).x);
    TEST_CHECK(pedModel.getPropulsionForces().at(i).y == pedModel.getPropulsionForces().at(i).y);
  }
}
}  // namespace Calm_Pedestrian_Model_TESTS
