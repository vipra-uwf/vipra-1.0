...
FLOATING_NUMBER CalmPedestrianModel::calculateDistance(int pedestrianIndexOfFirst, int pedestrianIndexOfSecond)
{
    ...
    FLOATING_NUMBER xDistance = pow((data->getPedestrianSet()->getPedestrianCoordinates()->at(x).at(pedestrianIndexOfFirst) - data->getPedestrianSet()->getPedestrianCoordinates()->at(x).at(pedestrianIndexOfSecond)), 2);
    FLOATING_NUMBER yDistance = pow((data->getPedestrianSet()->getPedestrianCoordinates()->at(y).at(pedestrianIndexOfFirst) - data->getPedestrianSet()->getPedestrianCoordinates()->at(y).at(pedestrianIndexOfFirst)), 2);
