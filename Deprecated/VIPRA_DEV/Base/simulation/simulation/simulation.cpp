...
void Simulation::setData(Data *data) { this->data = data; }
...
void Simulation::run() {
  ...
  // std::cout << "Adding " << this->timestep_size << " msec to the simulation."
  // << std::endl;
  while (
      i <
      15000) //! this->pedestrianDynamicsModel->getGoals()->isSimulationGoalMet())
  {
    ...
    // this->policyModel->update(this->timestep_size); // Reserved for future
    // use
...
int *Simulation::getTimestep() { return &this->timestep; }
...
void Simulation::printDataDELETETHIS() {
  ...
  CalmPedestrianSet *calmPedSet =
      dynamic_cast<CalmPedestrianSet *>(data->getPedestrianSet());
  ...
  for (int i = 0; i < calmPedSet->getNumPedestrians(); ++i) {
    std::cout
        << "ped [" << i << "] ("
        << calmPedSet->getPedestrianCoordinates().at(i)[0] << ", "
        << calmPedSet->getPedestrianCoordinates().at(i)[1] << ")"
        << " currgoal ("
        << this->pedestrianDynamicsModel->getGoals()->getCurrentGoal(i)[0]
        << ", "
        << this->pedestrianDynamicsModel->getGoals()->getCurrentGoal(i)[1]
        << ")"
        << " endGoal ("
        << this->pedestrianDynamicsModel->getGoals()->getEndGoal(i)[0] << ", "
        << this->pedestrianDynamicsModel->getGoals()->getEndGoal(i)[1] << ")"
        << " metGoal ("
        << std::to_string(
               this->pedestrianDynamicsModel->getGoals()->isPedestianGoalsMet(
                   i))
        << ") "
        << " velocity (" << calmPedSet->getVelocities().at(i)[0] << ", "
        << calmPedSet->getVelocities().at(i)[1] << ")"
        ...
        << " propulsion_forcex=" << calmPedSet->getPropulsionForces().at(i)[0]
        << " propulsion_forcey=" << calmPedSet->getPropulsionForces().at(i)[1]
        << " nearest_neighbor="
        ...
        << " priority=" << calmPedSet->getPriorities().at(i) << " Move_state=";
    state = calmPedSet->getMovementStates().at(i);
    switch (state) {
    case MovementDefinitions::PED_DYNAM:
      std::cout << "PED_DYNAM";
      break;
    case MovementDefinitions::HUMAN:
      std::cout << "HUMAN";
      break;
    case MovementDefinitions::POLICY:
      std::cout << "POLICY";
      break;
    case MovementDefinitions::STOP:
      std::cout << "STOP";
      break;
    }
    ...
    switch (status) {
    case RaceStatus::NO_RACE:
      std::cout << "NO_RACE";
      break;
    case RaceStatus::IN_RACE:
      std::cout << "IN_RACE";
      std::cout << " Opponent=" << calmPedSet->getOpponentIDs().at(i);
      break;
    case RaceStatus::WINNER:
      std::cout << "WINNER";
      break;
    case RaceStatus::LOOSER:
      std::cout << "LOOSER";
      break;
    }
    std::cout << std::endl;
