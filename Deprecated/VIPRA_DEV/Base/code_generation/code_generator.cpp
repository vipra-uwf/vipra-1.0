...
int main(int argc, char *argv[]) {
  ...
  mainFile << includes << functionDeclarations << getInputFiles << mainFunction
           << objectFunctions << extractConfigMap;
  ...
  for (const auto &option : jsonObj[type]) {
    if (option["compiled"]) {
      generatedFunction +=
          ...
          option["id"].asString() + "\"){\n\t\t" +
          option["className"].asString() + "* " + option["name"].asString() +
          " = new " + option["className"].asString() + "();\n\t\t" +
...
// NOTE: extractFileData takes in a string and a config_map* but does nothing
// with the pointer, so it's a nullptr for now -RG
std::string generatePopulateObstacleSet() {
  return {
      ...
      "\n\tentity_set_factory->populateObstacleSet(obstacleData, "
      "obstacle_set);" +
...
std::string generatePopulatePedestrianSet() {
  return {
      ...
      debugOutput("done reading") +
      "\n\tentity_set_factory->populatePedestrianSet(pedestrianData, "
      "pedestrian_set);" +
...
std::string runSim() {
  return {debugOutput("running simulation") +
          "\n\tsimulation->run();"
...
std::string generateMain() {
  ...
  for (const auto &[type, className] : TYPES) {
    mainFunction += "\n\tCONFIG_MAP* " + type +
                    "Config =  extractConfigMap(\"" + type + "\");";
  }

  for (const auto &[type, className] : TYPES) {
    mainFunction += "\n\t" + className + "* " + type + " = generate" +
                    className + "(simulationJsonConfig[\"" + type +
                    "\"].asString(), " + type + "Config);";
  }

  mainFunction += generatePopulateObstacleSet();

  mainFunction += generatePopulatePedestrianSet();
