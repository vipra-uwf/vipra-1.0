...
antlrcpp::Any DslHumanBehavior::visitEnvironmentInitialStateDeclaration(
...
antlrcpp::Any
DslHumanBehavior::visitStateTransitionEnvironmentState(BehaviorsParser::StateTransitionEnvironmentStateContext *ctx)
{
    std::string fromStateName = ctx->pedestrianSelected()->ID(1)->getText();
    int fromState = findState(this->getStateDefinitions(), fromStateName);
    ...
    SimulationContext *simulationContext = this->getSimulationContext();
    Condition *fromStateCondition = new StateCondition(simulationContext, fromState);
    Condition *transitionCondition = new EnvironmentStateCondition(this->getSimulationContext(), environmentStateValue);
...
antlrcpp::Any DslHumanBehavior::visitEnvironmentTransition(BehaviorsParser::EnvironmentTransitionContext *ctx)
{
    ...
    this->addEnvironmentTransition
    (
            new EnvironmentTransition
            (
                    this->getSimulationContext(),
                    new AndCondition
                    (
    ...
    std::cout << behavior << ": Added environment transition from "
              << fromStateName << " to " << toStateName
              << " after " << seconds << " seconds." << std::endl;
